...
public class MainController {
    ...
    private VBox buildSidebar() {
        ...
        transmissionSlider = labelledSlider("Transmission rate",
                0.01, 1.0, currentDisease.getTransmissionRate(), box);
        mortalitySlider    = labelledSlider("Mortality rate",
                0.0,  0.5, currentDisease.getMortalityRate(),    box);
        radiusSlider       = labelledSlider("Airborne radius",
                1, 8, currentDisease.getTransmissionRadius(),    box);
        ...
        susceptibleSlider = labelledSlider("Susceptible %", 0, 100, 40, box);
        infectedSlider    = labelledSlider("Infected %",    0, 100,  5, box);
    ...
    private void openSettings() {
        ...
        ConfigPanel.ConfigResult current = new ConfigPanel.ConfigResult(
                ...
                currentDisease,
                (int) susceptibleSlider.getValue(),
                (int) infectedSlider.getValue(),
        ...
        ConfigPanel.show(primaryStage, current, result -> {
            ...
            // Re-populate grid
            int total  = result.gridWidth() * result.gridHeight();
            int sCount = (int)(total * result.susceptiblePct() / 100.0);
            int iCount = (int)(total * result.infectedPct()    / 100.0);
            grid.randomPopulate(sCount, iCount);
            ...
            susceptibleSlider.setValue(result.susceptiblePct());
            infectedSlider   .setValue(result.infectedPct());
