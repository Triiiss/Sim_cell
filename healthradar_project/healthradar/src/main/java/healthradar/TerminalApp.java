...
public class TerminalApp {
    ...
    private boolean pauseMenu() {
        ...
        option("3", "Advance one step");
        option("4", "Restart  –  same settings");
        option("5", "Restart  –  reconfigure everything");
        option("6", "Change speed            (current: " + stepDelayMs + " ms/step)");
        option("7", "Change disease");
        option("8", "Toggle topology         (current: " + (toroidal ? "Toroidal" : "Bounded") + ")");
        option("9", "Save simulation");
        ...
        switch (prompt()) {
            ...
            case "3"      -> { engine.step(); clear(); printGridAndStats(); return true; }
            case "4"      -> { initSimulation(); return true; }
            case "5"      -> { configureGrid(); configureDisease();
                               configurePopulation(); initSimulation(); return true; }
            case "6"      -> changeSpeed();
            case "7"      -> { configureDisease(); grid.setDisease(disease); }
            case "8"      -> { toroidal = !toroidal; grid.setToroidal(toroidal);
                               ok("Topology: " + (toroidal ? "Toroidal" : "Bounded")); }
            case "9"      -> saveSimulation();
    ...
    /**
     * Step-by-step mode: ENTER advances one cycle, "q" exits.
     */
    private void stepByStepMode() {
        ...
        System.out.println("  " + FG_YELLOW + BOLD + "STEP-BY-STEP MODE" + RESET
                + "  –  ENTER = next step,  q = exit");
        while (true) {
            System.out.print("  [step " + engine.getStepCount() + "] > ");
            String line = readLine();
            if (line.equalsIgnoreCase("q")) { ok("Returning to auto-play."); return; }
            engine.step();
            clear();
            printGridAndStats();
            System.out.println();
